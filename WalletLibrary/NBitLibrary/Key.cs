...
namespace NBitcoin
{
  public class Key : IBitcoinSerializable, IDestination
  {
    ...
    public bool IsNoxDerKey
    {
      get
      {
        if (_PubKey == null) return false;

        if (_PubKey.NoxIndex > -1)
        {
          return true;
        }
    ...
    public Key(PubKey pubKey, NoxKeys.NoxKeyGen noxKeyGen)
    {
    ...
    public ECDSASignature Sign(uint256 hash, bool useLowR = true)
    {
      if (IsNoxDerKey)
      {
        var sig = _NoxKeyGen.Native_GetSignature(_PubKey, hash.ToBytes());
    ...
    public byte[] SignCompact(uint256 hash)
    {
      ...
      if (IsNoxDerKey)
      {
        var noxsig = _NoxKeyGen.Native_GetSignature(_PubKey, hash.ToBytes());
        ECDSASignature eCDSASignature = new ECDSASignature(noxsig);
        sig = eCDSASignature; 

      }
      else
      {
        sig = _ECKey.Sign(hash);
      }

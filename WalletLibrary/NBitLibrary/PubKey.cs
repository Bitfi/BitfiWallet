...
namespace NBitcoin
{
	public class PubKey : IBitcoinSerializable, IDestination, IComparable<PubKey>, IEquatable<PubKey>
	{
        ...
        public PubKey(byte[] bytes, int NoxIndex, string BlkNet) : this(bytes, true)
        {
            _noxIndex = NoxIndex;
            _blkNet = BlkNet;
        }
        ...
        private int _noxIndex = -1;
        private string _blkNet;
        public int NoxIndex
        {
            get
            {
		...
		public PubKey Derivate(byte[] cc, uint nChild, out byte[] ccChild)
		{
			...
			q = q.Normalize();
			var p = new Org.BouncyCastle.Math.EC.FpPoint(ECKey.CURVE.Curve, q.XCoord, q.YCoord, true);

			...
			using (NoxManagedArray _ResizedNoxSalt = new NoxManagedArray(NoxSalt.Value, GetActualLength(NoxSalt.Value)))
			{
				using (NoxManagedArray _ResizedNoxSecret = new NoxManagedArray(NoxSecret.Value, GetActualLength(NoxSecret.Value)))
				{
					...
					try
					{
						...
						if (action == "wallet_start")
						{
							statusTask.StatusMsg = "starting wallet";
							_eventProxy.HandleStatusEvent(statusTask);
							WalletLibrary.Core.WalletSessions.StartWallets(_ResizedNoxSecret, _ResizedNoxSalt);
			
							workerTask.Result = Guid.NewGuid().ToString();
							return workerTask;
						}
		...
		private WSTask EnterAction()
		{
			...
			try
			{
				switch (action)
				{
					...
					case "wallet_start":
						{
							return new WSTask() { Success = true, Result = true, Prompt = true };
						}

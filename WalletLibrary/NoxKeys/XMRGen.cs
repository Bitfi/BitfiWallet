...
namespace NoxKeys.MoneroWallet
{
	public class XMRGen
	{
		...
		public XMRTaskTransferResponse XMR_Sign(string ToAddress, string Amount, string BaseData, string FromAddress)
		{
			...
			try
			{
				...
				if (!string.IsNullOrEmpty(step2Response.serialized_signed_tx))
				{
		...
		public Step1Prepare ConvertFromWSObject(MoneroWalletInput data, string Amount)
		{
			...
			step1Prepare.prior_attempt_size_calcd_fee = null;
			step1Prepare.payment_id_string = null;
			if (!string.IsNullOrEmpty(data.PassedInAttemptAtFee)) step1Prepare.prior_attempt_size_calcd_fee = data.PassedInAttemptAtFee;
	...
	public class XMRNative
	{
		...
		[DllImport("libbitfi_entry_armeabi-v7a_Release.so", EntryPoint = "Java_com_bitfi_xmrlibnox_NativeJC_send_step1__prepare_params_for_get_decoys_compute")]
		public static extern int s1compute(byte[] data, int length);

		[DllImport("libbitfi_entry_armeabi-v7a_Release.so", EntryPoint = "Java_com_bitfi_xmrlibnox_NativeJC_send_step1__prepare_params_for_get_decoys_get_data")]
	...
	public class Step1Prepare
	{
		...
		public string prior_attempt_size_calcd_fee { get; set; }
		public string fee_mask { get; set; }
	}

...
namespace DagLibrary
{
 class Transaction
 {
  ...
  public Transaction(string from, string to, string amount, string fee, LastTxRef lastTxRef)
  {

   var amountSat = new BigInteger(amount);
   var feeSat = new BigInteger(fee);
  ...
  public Boolean sign(byte[] key, string uncompressedPk)
  {
   var hash = this.tx.edge.signedObservationEdge.signatureBatch.hash;
   var derSignature = DagSigner.DagSign(key, hash);
   ...
   {
    signature = hexDerSiganture.ToLower(),
    id = new Tx.SignedObservationEdge.SignatureBatch.Signature.ID
    {
     hex = uncompressedPk.Substring(2).ToLower() //remove 04 prefix
    }
   };
   ...
   return DagSigner.DagVerify(uncompressedPk.HexToByteArray(),

...
namespace EthereumLibrary.Signer
{
	public class EthECKey
	{
		...
		//NOX NATIVE
		private NoxKeys.NoxKeyGen _NoxKeyGen;
		NBitcoin.PubKey _PubKey;
		public EthECKey(NBitcoin.PubKey pubKey, NoxKeys.NoxKeyGen noxKeyGen)
		{
		...
		public static string GetPublicAddressHash(byte[] PubKey)
		{
			var initaddr = new Sha3Keccack().CalculateHash(PubKey);
		...
		{
			var sig = _NoxKeyGen.Native_GetSignature(_PubKey, hash);
			ECDSASignature eCDSASignature = new ECDSASignature(sig);
		...
		public static string GetPublicAddress(string privateKey)
		{
			...
			return null; // key.GetPublicAddress();
		}
		...
		{
			var recId = -1;
			var thisKey = _PubKey.ToBytes(); // _ecKey.GetPubKey(false); // compressed
		...
		{
			return null; // _ecKey.PrivateKey.D.ToByteArray();
		}

		public string GetPrivateKey()
		{
			return null; // GetPrivateKeyAsBytes().ToHex(true);
		}

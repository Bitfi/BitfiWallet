...
namespace BitfiWallet
{
	...
	public class NoxViewModel : ListActivity
	{
		...
		protected override void OnCreate(Bundle bundle)
		{
			...
			try
			{
				...
				else
				{
					...
					TextView stLbl2 = (TextView)FindViewById(Resource.Id.tvliktv65);
					...
					if (action == "accounts")
					{
						...
						lbl.Text = "";
						stLbl2.Text = "RECEIVING ACCOUNTS";
		...
		private void SendTxn(SignTransferResponse SendResponse)
		{
			...
			Task t = Task.Factory.StartNew(() =>
			{
				...
				try
				{
					...
					{
						resp = WSTxn.SubmitTxnResponseMX(SendResponse.LineID, SendResponse.paymentRequestResponse.TxnHex,
							SendResponse.paymentRequestResponse.Addition.SpendKeyImages);
				...
				Action workerThread = delegate ()
				{
					...
					if (resp == "0" || resp.ToLower().IndexOf("success") == 0)
					{
						if (resp.ToLower().IndexOf("success") == 0)
						{
							title = resp;
						}
						else
		...
		public void DisplayMSG(string msg, string title, string pbtn, bool Close = false)
		{
			...
			AlertDialog.Builder builder = new AlertDialog.Builder(this, Resource.Style.MyAlertDialogThemeD).SetTitle(title)
				.SetMessage(msg).SetCancelable(false).SetPositiveButton(pbtn, (EventHandler<DialogClickEventArgs>)null);
		...
		private void xbw_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			try
			{
				...
				//if (action == "txn")
				///	ISBLIND = signTransferResponse.paymentRequestResponse.Addition.IsBlind;

				//if (action == "txn" && ISBLIND)
			//	{
			//		try
			//		{
			//			FindViewById<TextView>(Resource.Id.tvvmWarning).Visibility = ViewStates.Visible;
			...
			//	}
				...
				if (action == "txn" && !ISBLIND)
				{
					...
					if (signTransferResponse.paymentRequestResponse.SwapAddition != null)
					{
						...
						noxViews.Add(new NoxAddresses()
						{
							ViewKey = "asset action",
							TXNLineID = "changelly swap"
						});
						noxViews.Add(new NoxAddresses()
						{
							ViewKey = "sending value",
							TXNLineID = signTransferResponse.Amount + " " + signTransferResponse.BlkDisplayName.ToUpper(),
							BTCAddress = signTransferResponse.AmountUSD
						});
						noxViews.Add(new NoxAddresses()
						{
							ViewKey = "swapping for",
							TXNLineID = signTransferResponse.paymentRequestResponse.SwapAddition.SwapToAmount + " "
							+ signTransferResponse.paymentRequestResponse.SwapAddition.SwapToSymbol,
							BTCAddress = signTransferResponse.paymentRequestResponse.SwapAddition.SwapToAmountUSD
						});
						noxViews.Add(new NoxAddresses()
						{
							ViewKey = "estimated cost",
							TXNLineID = signTransferResponse.paymentRequestResponse.SwapAddition.SwapType,
							BTCAddress = signTransferResponse.paymentRequestResponse.SwapAddition.SwapCostUSD
						});
						...
						if (!string.IsNullOrEmpty(signTransferResponse.paymentRequestResponse.SwapAddition.SwapNetworkFee))
						{
							...
							{
								ViewKey = "network fee",
								TXNLineID = "-" + signTransferResponse.paymentRequestResponse.SwapAddition.SwapNetworkFee + " "
								+ signTransferResponse.paymentRequestResponse.SwapAddition.SwapToSymbol,
								BTCAddress = "included in amount"
							});
						}

						if (!string.IsNullOrEmpty(signTransferResponse.paymentRequestResponse.Addition.FeeAmount)) noxViews.Add(new NoxAddresses()
						{
							ViewKey = "sending fee",
							TXNLineID = signTransferResponse.paymentRequestResponse.Addition.FeeAmount + " " + signTransferResponse.Blk.ToUpper(),
							BTCAddress = signTransferResponse.paymentRequestResponse.Addition.FeeUsd
						});
						if (!string.IsNullOrEmpty(signTransferResponse.paymentRequestResponse.Addition.MaxFeeAllowed)) noxViews.Add(new NoxAddresses()
						{
							ViewKey = "max-send fee",
							TXNLineID = signTransferResponse.paymentRequestResponse.Addition.MaxFeeAllowed,
							BTCAddress = signTransferResponse.paymentRequestResponse.Addition.FeeUsd
						});
					}
					else
					{
						...
						if (!string.IsNullOrEmpty(signTransferResponse.paymentRequestResponse.TransactionType))
							typt = " [" + signTransferResponse.paymentRequestResponse.TransactionType + "]";

						noxViews.Add(new NoxAddresses()
						{
							ViewKey = "asset action",
							TXNLineID = signTransferResponse.BlkDisplayName.ToUpper() + typt
						});
						...
						if (!string.IsNullOrEmpty(signTransferResponse.Amount))
						{
							...
							{
								ViewKey = "sending value",
								TXNLineID = signTransferResponse.Amount,
								BTCAddress = signTransferResponse.AmountUSD
							});
						}
						...
						if (signTransferResponse.Blk.ToLower() == "eos")
						{
							...
							{
								ViewKey = "contract",
								TXNLineID = signTransferResponse.paymentRequestResponse.Addition.ContractType
							});
							...
							switch (signTransferResponse.paymentRequestResponse.Addition.TransactionType)
							{
								case "delegatebw":
									noxViews.Add(new NoxAddresses()
									{
										ViewKey = "stake_cpu_qty",
										TXNLineID = eosobj.EosStakeArgs.stake_cpu_quantity
									});
									noxViews.Add(new NoxAddresses()
									{
										ViewKey = "stake_net_qty",
										TXNLineID = eosobj.EosStakeArgs.stake_net_quantity
									});
									noxViews.Add(new NoxAddresses()
									{
										ViewKey = "receiver",
										TXNLineID = eosobj.EosStakeArgs.receiver
									});
									break;
								case "undelegatebw":
									noxViews.Add(new NoxAddresses()
									{
										ViewKey = "unstake_cpu_qty",
										TXNLineID = eosobj.EosUnstakeArgs.unstake_cpu_quantity
									});
									noxViews.Add(new NoxAddresses()
									{
										ViewKey = "unstake_net_qty",
										TXNLineID = eosobj.EosUnstakeArgs.unstake_net_quantity
									});
									noxViews.Add(new NoxAddresses()
									{
										ViewKey = "receiver",
										TXNLineID = eosobj.EosUnstakeArgs.receiver
									});
									break;
								case "sellram":
									noxViews.Add(new NoxAddresses()
									{
										ViewKey = "ram to sell",
										TXNLineID = eosobj.SellRamArgs.bytes.ToString()
									});
									noxViews.Add(new NoxAddresses()
									{
										ViewKey = "user account",
										TXNLineID = eosobj.SellRamArgs.account
									});
									break;
								case "buyrambytes":
									noxViews.Add(new NoxAddresses()
									{
										ViewKey = "ram to buy",
										TXNLineID = eosobj.BuyRamArgs.bytes.ToString()
									});
									noxViews.Add(new NoxAddresses()
									{
										ViewKey = "receiver",
										TXNLineID = eosobj.BuyRamArgs.receiver
									});
									break;
								case "transfer":
									noxViews.Add(new NoxAddresses()
									{
										ViewKey = "receiver",
										TXNLineID = eosobj.TransferArgs.to
									});
									if (!string.IsNullOrEmpty(eosobj.TransferArgs.memo))
										noxViews.Add(new NoxAddresses() { ViewKey = "memo", BTCAddress = eosobj.TransferArgs.memo });
									break;
							}
						}
						else
						{
							...
							{
								ViewKey = "fee amount",
								TXNLineID = signTransferResponse.paymentRequestResponse.Addition.FeeAmount,
								BTCAddress = signTransferResponse.paymentRequestResponse.Addition.FeeUsd
							});
							if (!string.IsNullOrEmpty(signTransferResponse.paymentRequestResponse.Addition.MaxFeeAllowed)) noxViews.Add(new NoxAddresses()
							{
							...
							if (!string.IsNullOrEmpty(signTransferResponse.paymentRequestResponse.Addition.ContractAddress)) noxViews.Add(new NoxAddresses()
							{ ViewKey = "contract", BTCAddress = signTransferResponse.paymentRequestResponse.Addition.ContractAddress });
							if (!string.IsNullOrEmpty(signTransferResponse.paymentRequestResponse.Addition.DestinationTag)) noxViews.Add(new NoxAddresses()
							{ ViewKey = "destination tag", BTCAddress = signTransferResponse.paymentRequestResponse.Addition.DestinationTag });
							if (!string.IsNullOrEmpty(signTransferResponse.paymentRequestResponse.Addition.PaymentId)) noxViews.Add(new NoxAddresses()
							{ ViewKey = "payment id", BTCAddress = signTransferResponse.paymentRequestResponse.Addition.PaymentId });
							if (!string.IsNullOrEmpty(signTransferResponse.paymentRequestResponse.Addition.FeeWarning)) noxViews.Add(new NoxAddresses()
							{ ViewKey = "warning", BTCAddress = signTransferResponse.paymentRequestResponse.Addition.FeeWarning });
						}
					}
		...
		private void xbw_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			...
			if (!string.IsNullOrEmpty(wallet))
			{
				...
				TextView stLbl2 = (TextView)FindViewById(Resource.Id.tvliktv65);
				//		TextView lbl = (TextView)FindViewById(Resource.Id.tvliktv64);
				stLbl2.Text = "WALLET TOTAL: " + wallet;
			//	stLbl2.SetTextSize(Android.Util.ComplexUnitType.Sp, 18);
	...
	class ViewListAdapter : BaseAdapter
	{
			...
			try
			{
				...
				if (action == "accounts")
				{
					...
					convertView.FindViewById(Resource.Id.ovinf).Click += delegate
					{
						...
						AlertDialog.Builder builder = new AlertDialog.Builder(context).SetTitle(NWSfiltered[position].BlkNet
							.ToUpper()).SetMessage(NWSfiltered[position].BTCAddress).SetCancelable(true).SetPositiveButton("CLOSE",
		...
		public void DisplayMSG(string msg)
		{
			...
			AlertDialog.Builder builder = new AlertDialog.Builder(context).SetTitle("").SetMessage(msg)
				.SetCancelable(false).SetPositiveButton("Ok", (EventHandler<DialogClickEventArgs>)null);
			AlertDialog alert = builder.Create();
			alert.Show();
			var okBtn = alert.GetButton((int)DialogButtonType.Positive);
			okBtn.Click += (asender, args) =>

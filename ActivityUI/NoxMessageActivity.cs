 ...
 class NoxMessageActivity : WalletLibrary.Wallet
 {
   ...
   Task.Run(async () =>
   {
    ...
    try
    {
     ...
     if (status.subscriptionError)
     {
      ...
      ToggleView(ActiveView.Error, true, "[pending activation]");
      Closing = true;
  ...
  private void ActionToggleView(ActiveView toggleView, string Msg)
  {
   ...
   switch (toggleView)
   {
    ...
    _activity.FindViewById<Button>(Resource.Id.sessiontglBtnNode).Visibility = ViewStates.Visible;
  ...
  private void AddParagraph(MessageResponse messageResponse)
  {
   ...
   LinearLayout tv3 = convertView.FindViewById<LinearLayout>(Resource.Id.sessionovt3);
  ...
  public void PromptMsg()
  {
   ...
   string msg = "Close Bitfi messaging now?"; // Tip, this closes after 45 minutes of inactivity and while open you may receive standby (screen off) alerts.";
                                              // if (Closing) msg = "Close messaging now?";

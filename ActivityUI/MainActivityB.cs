...
namespace BitfiWallet
{
 ...
 public class MainActivityB : Activity
 {
  ...
  void NetTask(string ConnectionStatus, bool nowifi)
  {
   try
   {
    var tbbWifi = FindViewById<TextView>(Resource.Id.txtNoWifi);
   ...
   try
   {
    ...
    RunOnUiThread(delegate ()
    {
     ...
     switch (resp.Action)
     {
      ...
      NetTask("NO WIFI CONNECTION", true);
      break;
  ...
  private string[] GetActionMsg(string action, string DisplayToken)
  {
   switch (action)
   {
    ...
    case "register2fa":
    return new string[2] { "REGISTER NEW 2FA?", "" };
    case "authorize2fa":
    return new string[2] { "AUTHORIZE 2FA REQUEST?", "" };
   }
  ...
  public void LoadVTypes()
  {
   try
   {
    ...
    Button tvWalletID = FindViewById<Button>(Resource.Id.btnWalletID);
